...
class mympi:
    ...
    def write(self,data,fn=None):
        ...
        self.log.debug("write size %s masteroffset %s offset %s map %s"%(size,masteroffset,offset,map))
        ...
        try:
            ...
            if self.master:
                ...
                if len(header) > masteroffset:
                    self.log.error("Masteroffset %s smaller then real header length %s (%s)"%(masteroffset,len(header),header))
                fh.Write(header)
    ...
    def readbasic(self,fn,offset,bytestoread):
        if self.serial:
            try:
                ...
                self.log.error("Read serial: failed to open file %s: %s"%(fn,err))
        else:
            ...
            try:
                ...
                self.log.error("Read parallel: failed to open file %s: %s"%(fn,err))
    ...
    def runrun(self,cmd,returnout=False): #TODO replace with from vsc.utils.run import simple_run
        ...
        try:
            ...
            while ec < 0:
                ec=p.poll()
                ## need to read from time to time. otherwise the stdout/stderr buffer gets filled and it all stops working
            ...
            self.log.error("Something went wrong with forking cmd %s: %s"%(cmd,err))
    
        if returnout:
            return ec,out
        else:
            if ec > 0:
                self.log.error("Cmd '%s' failed: exitcode %s, output %s"%(cmd,ec,out))
